...
##agregar GUI
#agregar generadores aleatorios quanticos
...
def Main():
    ...
    while message != 'q':
        ...
        message = raw_input('-->')
        stringAlice = XorEncrypt(ForLoopEncrypt(message,int(llave,2)*blocks),int(llave,2))
        c.send(stringAlice)
        print "sending: " + stringAlice
        stringBob = c.recv(5000)
        Bobmessage = ForLoopDecrypt(XorDecrypt(stringBob,int(llave,2)),int(llave,2)*blocks)
